%% Single Mode Model .5cm with Background Potential 
%Window set V=[-5, 3.4]
%Decay parameter set to 2 instead of 3 for three filaments
%Flow feild not valid 
%Last edited by M. H. Kent Dec 21, 2021
...
%% 
% Grid
x = linspace(-1.25,1.25,256);
y = linspace(-1.25,1.25,256);
r = linspace(0,2,256);
[X,Y] = meshgrid(x,y);

%3 Filament Rotation and Distance   
th=pi/3.5; %Rotation from x=0
d = .5; %Distance of filaments from each ofther   
rr= d./sqrt(3);
%rr = .5;   %Distance of filaments from center of grid

% Time and Freqency of exparament
f1 = 73.6e3; %Will calculate base off of SHI filament 
...
C0=1;  %Set to '1' to use this potential or zero to negate
C1=-3.309;
C2=-.690;
C3=5.712;
C4=.397;
